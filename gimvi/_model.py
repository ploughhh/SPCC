...
from scvi.dataloaders._data_splitting import validate_data_split, DataSplitter
...
from ._spatial_field import SpatialField
...
def _unpack_tensors(tensors):
    ...
    # spatial_metric = tensors[REGISTRY_KEYS.SPATIAL_METRIC].squeeze_(0)
    # return x, batch_index, y, spatial_metric
...
class GIMVI(VAEMixin, BaseModelClass):
    ...
    **model_kwargs
        Keyword args for :class:`~scvi.external.gimvi_data.JVAE`
    ...
    def __init__(
        ...
        generative_distributions: List = ["zinb", "nb"], # ["zinb", "nb"]
        model_library_size: List = [True, False], # [True, False]
        n_latent: int = 10,
        ...
        **model_kwargs,
        ...
        n_celltypes = sum_stats[0]['n_labels']
    ...
    def train(
        ...
        celltype_classifier: Optional[bool] = True,
        **kwargs,
    ):
        """
        Train the model.
    ...
    def get_latent_representation(
        ...
        for mode, scdl in enumerate(scdls):
            ...
            for tensors in scdl:
                (
                    # x, batch_index, y, spatial_metric
                    x, batch_index, y
                ) = _unpack_tensors(tensors)
                latent.append(
                    self.module.sample_from_posterior_z(
                        # x, spatial_metric, mode, deterministic=deterministic
                        x, mode, deterministic=deterministic
                    )
                )
    ...
    def load_model(
        ...
        model_path = os.path.join(dir_path, f"{prefix}model.pt")
    ...
    def setup_anndata(
        ...
        **kwargs,
        ...
        anndata_fields = [
            ...
            # SpatialField(REGISTRY_KEYS.SPATIAL_METRIC, spatial_metric)
        ]
