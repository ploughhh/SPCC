...
from scvi.module.base._pyro import AutoMoveDataPredictive
...
class LossRecorder:
    ...
    **kwargs
    ...
    def __init__(
        ...
        # kl_divergence_gcn_z: Optional[LossRecord] = None,
        feature_loss: Optional[LossRecord] = None,
        **kwargs,
    ):
        ...
        # self._kl_divergence_gcn_z = (
        #     kl_divergence_gcn_z if isinstance(kl_divergence_gcn_z, dict) else dict(kl_divergence_gcn_z=kl_divergence_gcn_z)
        # )
        self._feature_loss = (
            feature_loss if isinstance(feature_loss, dict) else dict(feature_loss=feature_loss)
        )
    ...
    # @property
    # def kl_divergence_gcn_z(self) -> Union[torch.Tensor, jnp.ndarray]:
    #     return self._get_dict_sum(self._kl_divergence_gcn_z)

    # @property
    # def feature_loss(self) -> Union[torch.Tensor, jnp.ndarray]:
    #     return self._get_dict_sum(self._feature_loss)
...
class BaseModuleClass(nn.Module):
    ...
    def forward(
        ...
        # get_reconstruction_input_kwargs: Optional[dict] = None,
        ...
        get_reconstruction_input_kwargs
...
def _generic_forward(
    ...
    # reconstruction_kwargs = _get_dict_if_none(reconstruction_kwargs)
    loss_kwargs = _get_dict_if_none(loss_kwargs)
    get_inference_input_kwargs = _get_dict_if_none(get_inference_input_kwargs)
    get_generative_input_kwargs = _get_dict_if_none(get_generative_input_kwargs)
    # get_reconstruction_input_kwargs = _get_dict_if_none(get_reconstruction_input_kwargs)
##############################################################################################
    inference_inputs_x, inference_inputs_spatial = module._get_inference_input(
        tensors
    )
    inference_outputs = module.inference(**inference_inputs_x, **inference_inputs_spatial, **inference_kwargs)
    ...
    # reconstruction_inputs = module._get_reconstruction_input(
    #     tensors, inference_outputs, **get_reconstruction_input_kwargs
    # )

    # reconstruction_outputs = module.reconstruction(**reconstruction_inputs, **reconstruction_kwargs)
    if compute_loss:
        losses = module.loss(
